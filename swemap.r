...
read.content <- list()
read.content$data <- 
  matrix(0, ncol = 1, nrow = 5000) %>%
  as.data.frame()
use_raw <- data.frame()
iter <- 1 #Iter variable to indicate the query number
while(nrow(read.content$data) == 5000){
  path <- "/api/v1/sv/lakemedel/resultat/matt/3/atc/J01/region/1,3,4,5,6,7,8,9,10,12,13,14,17,18,19,20,21,22,23,24,25/ar/2016,2017"#database = "lakemedel", output = "resultat", measure = "matt 3", class = "atc J01", region = all regions by numbers, year = "ar 2017"
...
iter <- 1 #Iter variable to indicate the query number
while(nrow(read.content$data) == 5000){
  path <- "/api/v1/sv/lakemedel/resultat/matt/9/atc/J01/region/1,3,4,5,6,7,8,9,10,12,13,14,17,18,19,20,21,22,23,24,25/ar/2016,2017"#database = "lakemedel", output = "resultat", measure = "matt 3", class = "atc J01", region = all regions by numbers, year = "ar 2017"
  raw.search <- GET(url = url, path = path)
  read.content <- 
    raw.search$content %>%
    rawToChar() %>%
...
#Read shapefile and turn into tidy format
...
#Update regional codes and map data to shapefile
...
gg <- ggplot() + geom_polygon(data=shapefile_df, aes(x=long, y=lat, group = group, fill = shapefile_df$exp_per_1000), size = 0.1, colour="black") + 
                 scale_fill_gradient(name = str_wrap("Prescriptions/1000 inhabitants", 20), low = "steelblue1", high = "midnightblue", guide = "colourbar") + 
                 labs(title=str_wrap("Regional antibiotics consumption in Sweden, all ages, both sexes for 2017", 45), y="", x="", caption = caption_text) + 
                 ...
                 theme(plot.title=element_text(size=24, face="bold", lineheight=1.2),
